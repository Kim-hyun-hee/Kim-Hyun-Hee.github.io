...
처음에는 두 곳 모두에 두르고(글 헤더는 2px) 실물로 비교했다. 사이드바만 두른
쪽이 나았다. 링이 사이드바 전용이 되면서 "여기가 나를 소개하는 자리"라는 역할이
선명해지고, 본문 프로필은 글을 읽는 흐름에 끼어 있어 장식이 적은 편이 낫다.

`profile-ring` 유틸리티는 그대로 둔다. 호출부가 하나로 줄었을 뿐이다.
...
맞춰야 하는데 사이드바는 `--muted`, 본문은 `--background`라 값이 갈린다.
...
3. 사이드바 사진에 그라데이션 링이 보이는지 — 띠가 아니라 원인지. 글 헤더
   사진에는 링이 **없어야** 한다
