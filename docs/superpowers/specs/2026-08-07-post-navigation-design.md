...
### 9.1 닫은 결정 (2026-08-07)

아래 둘은 이 설계를 실물로 확인한 뒤 **하지 않기로 확정했다.** 다시 열지
않는다.

- **시리즈 아코디언 재설계** — 하단 상자가 전체 목록을 항상 펼치게 되면서
  접기/펼치기라는 문제 자체가 없어졌다. 아코디언으로 바꿀 대상이 남아 있지
  않다.
- **`#9b` 이전/다음 글 플로팅 카드** — 인라인 이전/다음 글을 없앤 자리를
  "카테고리의 다른 글"이 대신한다. 시간순 이웃을 화면 좌우 하단에 띄울
  이유가 사라졌고, 우하단 플로팅 버튼과의 자리 다툼도 그대로 남아 있다.
