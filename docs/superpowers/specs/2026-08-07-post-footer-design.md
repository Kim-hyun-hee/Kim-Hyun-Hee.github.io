...
- **시리즈 UI 일체.** `SeriesBox`(본문 위)와 `SeriesNav`(하단)의 생김새를
  아코디언 형태로 다시 짤 예정이라, 지금 맞춰두면 그때 다시 뜯게 된다.
  이번 작업은 두 파일을 열지 않는다. 아래 두 가지가 그때 함께 딸려간다:
  - `SeriesBox`·`SeriesNav`에 `data-pagefind-ignore`가 없어서 시리즈 글들이
    서로의 제목을 검색 색인에 흘리고 있다 (§5.4와 같은 문제)
  - 아코디언 구성 참고: <https://github.com/dpcivl/dev-blog>
...
- `size="lg"`(태그 목록 페이지) → `text-base`. **여백·모서리·아이콘 크기는
  `sm`과 완전히 같게 두고 글자 크기만 다르게 한다.** 두 페이지의 칩이 한
  부품으로 보이고, 나중에 색이나 모서리를 손볼 때 고칠 곳이 한 군데로 남는다.
  `/tags`의 태그가 지금(`text-lg`)보다 조금 작아지는 것은 감수한다.
...
3. `current`가 시리즈에 속해 있으면 **같은 시리즈의 다른 글을 뺀다**
   (`current` 자신은 다음 단계에서 위치를 찾아야 하므로 남긴다). 이유는 §5.7.
4. 좁힌 목록에서 `current`의 위치를 찾아 **현재 글을 가운데 둔 창**을 잡고,
...
- 시리즈에 속한 글 → 같은 시리즈의 다른 편이 섞이지 않고, 시리즈 밖 글로만 채움
- 범위 안이 전부 같은 시리즈일 때 → 빈 배열
...
- **`limit`으로 4를 넘긴다.** 유틸의 기본값은 5지만, 4를 쓰면 창 크기가
  홀수(4+1)가 되어 현재 글 앞뒤로 2개씩 정확히 대칭이 된다. 기본값 5는
  창이 짝수(6)라 최신 쪽으로 한 개 치우친다.

### 5.7 시리즈 글에서의 동작

시리즈 글에서도 이 블록은 그대로 뜬다. 다만 §5.2의 3단계에서 **같은 시리즈의
다른 편은 목록에서 뺀다.**

빼지 않으면 같은 링크가 한 화면에 세 번 나온다. 3편짜리 연재의 3편을 읽는
독자에게 본문 위 `SeriesBox`가 1·2편을 보여주고, 하단 `SeriesNav`가 2편을 또
보여주고, 이 블록이 1·2편을 다시 보여주는 식이다. 소분류가 없는 대분류
(Project·Etc)에서 특히 심하다 — 범위가 대분류 전체인데 그 안이 연재뿐이면 이
블록이 사실상 시리즈 목차의 복사본이 된다.

빼고 나면 각 블록의 역할이 갈린다.

| 블록 | 답하는 질문 |
|---|---|
| `SeriesBox` / `SeriesNav` | 이 연재의 다른 편은? |
| 카테고리의 다른 글 | **이 연재 말고** 같은 분류에 뭐가 있나? |
| 이전 글 / 다음 글 | 블로그 전체에서 시간순 이웃은? |

뺀 결과 남는 글이 없으면 컴포넌트가 아무것도 렌더하지 않으므로(§5.4) 빈 상자가
생길 일은 없다.

이 조건은 유틸 안에만 있고 시리즈 컴포넌트를 건드리지 않으므로, 나중에 시리즈
UI를 아코디언으로 다시 짜도 그대로 유효하다(§2 참고).
...
- 소분류가 있는 글에서 같은 소분류 글만 최대 4개(앞뒤 2개씩) 뜬다
- 소분류가 없는 글에서 같은 대분류 글이 최대 4개 뜬다
- 카테고리에 글이 하나뿐이면 블록이 아예 렌더되지 않는다
- 시리즈 글에서 이 블록에 같은 연재의 다른 편이 뜨지 않는다
- `SeriesBox.astro`·`SeriesNav.astro`는 이번 diff에 등장하지 않는다
